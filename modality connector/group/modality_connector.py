...
from .group_attention import GroupCrossTransformerEncoderLayer, GroupCrossTransformerEncoder
...
class VisionEncoder(nn.Module):
    ...
    def __init__(self, config: AuroraConfig):
        ...
        self.cross_vision = GroupCrossTransformerEncoder(
            GroupCrossTransformerEncoderLayer(
...
class TextEncoder(nn.Module):
    ...
    def __init__(self, config: AuroraConfig):
        ...
        self.cross_text = GroupCrossTransformerEncoder(
            GroupCrossTransformerEncoderLayer(
                d_model=config.hidden_size,
                nhead=config.num_attention_heads,
                dim_feedforward=config.intermediate_size,
...
class ModalityConnector(nn.Module):
    def __init__(self, config: AuroraConfig):
        ...
        # Define learnable target tokens (shape: [num_distill_tokens, hidden_size])
        self.connect_text = GroupCrossTransformerEncoder(
            GroupCrossTransformerEncoderLayer(
                d_model=config.hidden_size,
                nhead=config.num_attention_heads,
                dim_feedforward=config.intermediate_size,
        ...
        self.connect_vision = GroupCrossTransformerEncoder(
            GroupCrossTransformerEncoderLayer(
                d_model=config.hidden_size,
                nhead=config.num_attention_heads,
                dim_feedforward=config.intermediate_size,
